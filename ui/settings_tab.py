...
class SettingsTab(BaseTab):
    ...
    def create_widgets(self):
        ...
        dbt_frame = ctk.CTkFrame(self, corner_radius=theme.CORNER_RADIUS, fg_color=theme.colors["sidebar"])
        dbt_frame.grid(row=1, column=0, sticky='ew', pady=(0, 15), padx=5)
        dbt_frame.columnconfigure(1, weight=1)

        ctk.CTkLabel(dbt_frame, text="Configurações dbt", font=theme.fonts["h1"]).grid(row=0, column=0, columnspan=2, sticky='w', padx=15, pady=(10,5))
        ctk.CTkLabel(dbt_frame, text="Caminho Projeto dbt:", font=theme.fonts["body"]).grid(row=1, column=0, sticky='w', padx=15, pady=(5, 15))
        self.dbt_path_var = ctk.StringVar()
        ctk.CTkEntry(dbt_frame, textvariable=self.dbt_path_var, fg_color=theme.colors["background"], border_color=theme.colors["comment"]).grid(row=1, column=1, sticky='ew', padx=15, pady=(5, 15))
    ...
    def load_settings_to_ui(self):
        ...
        self.dbt_path_var.set(self.config_manager.get("dbt_project_path"))
    ...
    def save_settings(self):
        ...
        self.config_manager.set("dbt_project_path", self.dbt_path_var.get())
